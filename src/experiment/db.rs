use crate::experiment::bins::SliceExt;
use crate::experiment::btree::{BTree, Cursor};
use crate::experiment::id::{PageId, PageIdExt, TxId};
use crate::experiment::log::{TxState, WalEntry, WalKind};
use crate::experiment::pager::{LogContext, PageOps, Pager};
use crate::experiment::recovery::{recover, undo_txn};
use crate::experiment::runtime::{
    Atomic, File, JoinHandle, Runtime, RwMutex, RwMutexReadGuard, RwMutexWriteGuard, Timer,
    TimerHandle,
};
use crate::experiment::wal::Wal;
use anyhow::anyhow;
...
impl<'db, R: Runtime> Tx<'db, R> {
    ...
    pub fn bucket(&mut self, name: &str) -> anyhow::Result<Bucket<R>> {
        ...
        let mut btree = crate::experiment::btree::new(self.id, &self.pager, &self.wal, root_pgid);
        ...
        Ok(Bucket {
            btree: crate::experiment::btree::new(self.id, &self.pager, &self.wal, bucket_pgid),
        })
    }
...
impl<'db, R: Runtime> ReadTx<'db, R> {
    ...
    pub fn bucket(&self, name: &str) -> anyhow::Result<Option<ReadBucket<R>>> {
        ...
        let btree = crate::experiment::btree::new(self.txid, &self.pager, &self.wal, root_pgid);
        let result = btree.get(name.as_bytes())?;
        ...
        Ok(Some(ReadBucket {
            btree: crate::experiment::btree::new(self.txid, &self.pager, &self.wal, pgid),
        }))
    }
}
