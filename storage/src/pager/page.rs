...
pub(crate) trait PageWriteOps<'a>: PageOps<'a> {
    ...
    async fn init_interior<R: Runtime>(
        mut self,
        ctx: LogContext<'_, R>,
        last: PageId,
        ...
        if let PageKind::None = self.internal().meta.kind {
            ...
            internal.meta.lsn = ctx.record_interior_init(internal.txid, pgid, last).await?;
    ...
    async fn set_interior<R: Runtime>(
        mut self,
        ctx: LogContext<'_, R>,
        payload: &'a [u8],
    ) -> anyhow::Result<InteriorPageWrite<Self>> {
        ...
        internal.meta.lsn = ctx
            .record_interior_set(internal.txid, pgid, Bytes::new(payload))
            .await?;
        internal.meta.dirty = true;
    ...
    async fn init_leaf<R: Runtime>(
        ...
        if let PageKind::None = self.internal().meta.kind {
            let pgid = self.id();
            let internal = self.internal_mut();
            internal.meta.lsn = ctx.record_leaf_init(internal.txid, pgid).await?;
            internal.meta.dirty = true;
    ...
    async fn set_leaf<R: Runtime>(
        ...
        let internal = self.internal_mut();
        internal.meta.lsn = ctx
            .record_leaf_set(internal.txid, pgid, Bytes::new(payload))
            .await?;
        internal.meta.dirty = true;
    ...
    async fn init_overflow<R: Runtime>(
        mut self,
        ctx: LogContext<'_, R>,
        ...
        if let PageKind::None = self.internal().meta.kind {
            let pgid = self.id();
            let internal = self.internal_mut();
            internal.meta.lsn = ctx.record_overflow_init(internal.txid, pgid).await?;
            internal.meta.dirty = true;
...
where
    T: PageWriteOps<'a>,
{
    pub(crate) async fn reset<R: Runtime>(mut self, ctx: LogContext<'_, R>) -> anyhow::Result<T> {
        let pgid = self.id();
        let internal = self.internal_mut();
        ...
        internal.meta.lsn = ctx
            .record_interior_reset(internal.txid, pgid, payload)
            .await?;
        internal.meta.dirty = true;
    ...
    pub(crate) async fn set_last<R: Runtime>(
        &mut self,
        ctx: LogContext<'_, R>,
        new_last: PageId,
    ) -> anyhow::Result<()> {
    ...
    pub(crate) async fn set_cell_ptr<R: Runtime>(
        &mut self,
        ctx: LogContext<'_, R>,
        index: usize,
        ptr: PageId,
    ...
    pub(crate) async fn set_cell_overflow<R: Runtime>(
        &mut self,
        ctx: LogContext<'_, R>,
        index: usize,
    ...
    pub(crate) async fn insert_cell<R: Runtime>(
        &mut self,
        ctx: LogContext<'_, R>,
        i: usize,
        cell: InteriorCell<'_>,
    ...
    pub(crate) async fn insert_content<R: Runtime>(
        &mut self,
        ctx: LogContext<'_, R>,
        i: usize,
    ...
    T: PageWriteOps<'a>,
{
    pub(crate) async fn reset<R: Runtime>(mut self, ctx: LogContext<'_, R>) -> anyhow::Result<T> {
        ...
        let payload = Bytes::new(internal.buffer.payload());
        internal.meta.lsn = ctx.record_leaf_reset(internal.txid, pgid, payload).await?;
        internal.meta.dirty = true;
        internal.meta.kind = PageKind::None;
        Ok(self.0)
    }
    ...
    pub(crate) async fn set_next<R: Runtime>(
        &mut self,
        ctx: LogContext<'_, R>,
    ...
    T: PageWriteOps<'a>,
{
    ...
    pub(crate) async fn set_content<R: Runtime>(
        &mut self,
        ctx: LogContext<'_, R>,
        content: &mut impl Content,
        next: Option<PageId>,
    ...
    pub(crate) async fn unset_content<R: Runtime>(
        &mut self,
        ctx: LogContext<'_, R>,
    ) -> anyhow::Result<()> {
        let pgid = self.id();
