...
pub(crate) async fn recover<R: Runtime>(
    ...
    undo(pager, &wal, &analyze_result).await?;
    ...
    log::debug!("aries_recover finish");
...
impl<'a, R: Runtime> Redoer<'a, R> {
    ...
    async fn redo(&mut self, lsn: Lsn, entry: &WalEntry<'_>) -> anyhow::Result<()> {
        match entry.kind {
            WalKind::Begin { .. }
            | WalKind::Commit { .. }
            | WalKind::Rollback { .. }
            ...
            | WalKind::HeaderSetForUndo {
                ...
                self.pager
                    .set_state(LogContext::Redo(lsn), |state| {
                        *state = DbState {
                            root,
                            freelist,
                            page_count,
            ...
            WalKind::AllocPage { txid, pgid } => {
                let page = self.pager.alloc(LogContext::Redo(lsn), txid).await?;
                assert_eq!(page.id(), pgid);
                page.release().await;
            }
            WalKind::DeallocPage { txid, pgid } => {
                self.pager
                    .dealloc(LogContext::Redo(lsn), txid, pgid)
                    .await?;
            }
            ...
            | WalKind::OverflowSetNext { txid, pgid, .. } => {
                self.redo_page(lsn, entry, txid, pgid).await?;
            }
        }
    ...
    async fn redo_page(
        ...
        let page = self.pager.write(&ctx, txid, pgid).await?;
        if page.lsn() >= lsn {
            log::debug!(
        ...
        match entry.kind {
            ...
            WalKind::InteriorReset { .. } | WalKind::InteriorResetForUndo { .. } => {
                ...
                let page = page.reset(ctx).await?;
                page.release().await;
            }
            WalKind::InteriorSet { payload, .. } => {
                let page = page.set_interior(ctx, payload.slice()).await?;
                page.release().await;
            }
            WalKind::InteriorInit { last, .. } => {
                let page = page.init_interior(ctx, last).await?;
                if page.is_none() {
            ...
            WalKind::InteriorInsert {
                ...
                let ok = page
                    .insert_content(ctx, index, &mut raw, key_size, ptr, overflow)
                    .await?;
                page.release().await;
                if !ok {
                    return Err(anyhow!(
            ...
            WalKind::InteriorSetCellOverflow {
                ...
                page.set_cell_overflow(ctx, index, overflow).await?;
                page.release().await;
            }
            WalKind::InteriorSetCellPtr { index, ptr, .. } => {
                ...
                page.set_cell_ptr(ctx, index, ptr).await?;
                page.release().await;
            }
            ...
            WalKind::LeafSet { payload, .. } => {
                let page = page.set_leaf(ctx, payload.slice()).await?;
                page.release().await;
            }
            WalKind::LeafInit { .. } => {
                let page = page.init_leaf(ctx).await?;
                page.release().await;
            }
            WalKind::LeafInsert {
                index,
                ...
                let ok = page
                    .insert_content(ctx, index, &mut raw, key_size, value_size, overflow)
                    .await?;
                page.release().await;
                if !ok {
                    return Err(anyhow!(
            ...
            WalKind::OverflowInit { .. } => {
                let page = page.init_overflow(ctx).await?;
                if page.is_none() {
            ...
            WalKind::OverflowSetContent { next, mut raw, .. } => {
                ...
                page.set_content(ctx, &mut raw, next).await?;
                page.release().await;
            }
            WalKind::OverflowSetContentForUndo { .. } => {
                let Some(mut page) = page.into_write_overflow() else {
...
async fn undo<R: Runtime>(
    ...
    match &mut active_tx {
        ...
        TxState::Active(txid) => {
            ...
            undo_txn(pager, wal, *txid, &mut last_undone).await?;
        ...
        TxState::Aborting {
            ...
            undo_txn(pager, wal, *txid, last_undone).await?;
            wal.append_log(WalEntry {
                clr: None,
                kind: WalKind::End { txid: *txid },
            })
...
pub(crate) async fn undo_txn<R: Runtime>(
    pager: &Pager<R>,
    wal: &Wal<R>,
    txid: TxId,
    last_undone: &mut Lsn,
    ...
    let mut iter = wal.iter_back(*last_undone).await?;
    while let Some((lsn, entry)) = iter.next().await? {
        ...
        if let Some(last_clr) = last_clr {
            if lsn >= last_clr {
                log::debug!(
                    "undo_item skipped since lsn >= last_clr. lsn={lsn:?} last_clr={last_clr:?}"
                );
        ...
        match entry.kind {
            ...
            WalKind::AllocPage { txid, pgid } => {
                pager.dealloc(ctx, txid, pgid).await?;
            }
            WalKind::DeallocPage { .. } => {
            ...
            WalKind::InteriorReset {
                ...
                let page = pager.write(wal, txid, pgid).await?;
                let page = page.set_interior(ctx, payload.slice()).await?;
                page.release().await;
            }
            WalKind::InteriorResetForUndo { .. } => {
            ...
            WalKind::InteriorSetCellOverflow {
                ...
                page.set_cell_overflow(ctx, index, old_overflow).await?;
                page.release().await;
            }
            WalKind::InteriorSetCellPtr {
                pgid,
                ...
                page.set_cell_ptr(ctx, index, old_ptr).await?;
                page.release().await;
            }
            WalKind::InteriorSetLast { pgid, old_last, .. } => {
                ...
                page.set_last(ctx, old_last).await?;
                page.release().await;
            }
            ...
            WalKind::LeafSetNext { pgid, old_next, .. } => {
                ...
                page.set_next(ctx, old_next).await?;
                page.release().await;
            }
            ...
            WalKind::OverflowReset {
                ...
                let page = pager.write(wal, txid, pgid).await?;
                let page = page.set_overflow(ctx, payload.slice())?;
                page.release().await;
            }
            WalKind::OverflowResetForUndo { .. } => {
