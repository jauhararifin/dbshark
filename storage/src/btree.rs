...
impl<'a, R: Runtime> BTree<'a, R> {
    pub(crate) async fn put(&mut self, key: &[u8], value: &[u8]) -> anyhow::Result<()> {
        ...
        if self.put_fast(key, value).await? {
            return Ok(());
        }
        self.put_slow(key, value).await
    }

    async fn put_fast(&self, _key: &[u8], _value: &[u8]) -> anyhow::Result<bool> {
    ...
    async fn put_slow(&mut self, key: &[u8], value: &[u8]) -> anyhow::Result<()> {
        let mut result = self.lookup_for_insert(key).await?;
        ...
        let inserted = self
            .insert_content_to_leaf(&mut result.leaf.node, result.leaf.index, keyval, key.len())
            .await?;
        if inserted {
            return Ok(());
        }

        let new_right_page = self.new_page().await?;
        let mut new_right_leaf = new_right_page.init_leaf(self.ctx).await?;
    ...
    async fn lookup_for_insert(&self, key: &[u8]) -> anyhow::Result<LookupForUpdateResult<'a, R>> {
        ...
        let page = loop {
            ...
            let (i, found) = self.search_key_in_interior(&node, key).await?;
        ...
        let node = page.init_leaf(self.ctx).await?;

        let (i, found) = self.search_key_in_leaf(&node, key).await?;
    ...
    async fn delete_leaf_cell(
        &self,
        page: &mut LeafPageWrite<PageWrite<'_, R>>,
        index: usize,
    ) -> anyhow::Result<()> {
        ...
        page.delete(self.ctx, index).await?;
        Ok(())
    }

    async fn insert_content_to_leaf(
        &self,
        node: &mut LeafPageWrite<PageWrite<'_, R>>,
        index: usize,
        ...
        let ok = node
            .insert_content(self.ctx, index, &mut content, key_size, value_size, None)
            .await?;
        if !ok {
            return Ok(false);
        }
        ...
        let next_page = self.new_page().await?;
        let mut overflow = next_page.init_overflow(self.ctx).await?;
        let next_pgid = overflow.id();
        node.set_cell_overflow(self.ctx, index, Some(next_pgid))
            .await?;
        overflow.set_content(self.ctx, &mut content, None).await?;

        while !content.is_finished() {
            let next_page_2 = self.new_page().await?;
            let mut overflow_2 = next_page_2.init_overflow(self.ctx).await?;
    ...
    async fn insert_content_to_interior(
        &self,
        node: &mut InteriorPageWrite<PageWrite<'_, R>>,
        index: usize,
        ...
        let ok = node
            .insert_content(self.ctx, index, content, key_size, ptr, None)
            .await?;
        if !ok {
            return Ok(false);
        }
        ...
        node.set_cell_overflow(self.ctx, index, Some(next_pgid))
            .await?;
        overflow.set_content(self.ctx, content, None).await?;
    ...
    async fn leaf_split_and_insert<'b>(
        &self,
        left_leaf: &mut LeafPageWrite<PageWrite<'_, R>>,
        new_right_leaf: &'b mut LeafPageWrite<PageWrite<'_, R>>,
        index: usize,
        key: &[u8],
        value: &[u8],
        ...
        'a: 'b,
    {
        ...
        let (n_cells_to_keep, mut iter) = left_leaf.split(self.ctx).await?;
        while let Some(cell) = iter.next() {
            new_right_leaf.insert_cell(self.ctx, i, cell).await?;
            i += 1;
        }

        new_right_leaf.set_next(self.ctx, left_leaf.next()).await?;
        left_leaf
            .set_next(self.ctx, Some(new_right_leaf.id()))
            .await?;
        ...
        if index < n_cells_to_keep {
            ...
            self.insert_content_to_leaf(new_right_leaf, index - n_cells_to_keep, keyval, key.len())
                .await?;
        };
    ...
    async fn split_root_leaf(
        &self,
        a: LeafPageWrite<PageWrite<'_, R>>,
        mut b: LeafPageWrite<PageWrite<'_, R>>,
        c: PageId,
        pivot: &mut impl Content,
    ) -> anyhow::Result<()> {
        for i in 0..a.count() {
            let cell = a.get(i);
            b.insert_cell(self.ctx, i, cell).await?;
        }
        let a = a.reset(self.ctx).await?;
        b.set_next(self.ctx, Some(c)).await?;

        let mut a = a.init_interior(self.ctx, c).await?;
    ...
    async fn propagate_interior_splitting(
        &mut self,
        mut interiors: Vec<LookupHop<InteriorPageWrite<PageWrite<'_, R>>>>,
        ...
        let ptr = if i == page.count() {
            let last = page.last();
            page.set_last(self.ctx, right_pgid).await?;
            last
        } else {
            let ptr = page.get(i).ptr();
            page.set_cell_ptr(self.ctx, i, right_pgid).await?;
            ptr
        };

        let key_size = pivot.remaining();
        let inserted = self
            .insert_content_to_interior(&mut page, i, &mut pivot, ptr, key_size)
            .await?;
        if inserted {
            return Ok(());
        }
        ...
        let new_right_page = self.new_page().await?;
        let mut new_right_interior = new_right_page.init_interior(self.ctx, page.last()).await?;
        right_pgid = new_right_interior.id();

        let mut pivot = self
            .interior_split_and_insert(&mut page, &mut new_right_interior, i, pivot, ptr)
            .await?;
    ...
    async fn interior_split_and_insert<'b>(
        &mut self,
        left_interior: &mut InteriorPageWrite<PageWrite<'_, R>>,
        new_right_interior: &'b mut InteriorPageWrite<PageWrite<'_, R>>,
        index: usize,
        mut pivot: impl Content,
        ptr: PageId,
    ) -> anyhow::Result<BTreeContent<'b, R>>
        ...
        'a: 'b,
    {
        ...
        let mut iter = left_interior.split(self.ctx).await?;
        let n_cells_to_keep = iter.0;
        while let Some(cell) = iter.1.next() {
            if next_pivot.is_none() {
                ...
                new_right_interior.insert_cell(self.ctx, i, cell).await?;
                i += 1;
            }
        }
        ...
        new_right_interior
            .set_last(self.ctx, left_interior.last())
            .await?;
        left_interior.set_last(self.ctx, next_pivot.ptr).await?;
        ...
        match index.cmp(&n_cells_to_keep) {
            Ordering::Less | Ordering::Equal => {
                let ok = self
                    .insert_content_to_interior(left_interior, index, &mut pivot, ptr, key_size)
                    .await?;
                assert!(ok);
            }
            Ordering::Greater => {
                let ok = self
                    .insert_content_to_interior(
                        new_right_interior,
    ...
    async fn split_interior_root(
        &self,
        a: InteriorPageWrite<PageWrite<'_, R>>,
        b: PageWrite<'_, R>,
        c: PageId,
        pivot: &mut impl Content,
    ) -> anyhow::Result<()> {
        let a_last = a.last();
        let mut b = b.init_interior(self.ctx, a_last).await?;
        for i in 0..a.count() {
            let cell = a.get(i);
            b.insert_cell(self.ctx, i, cell).await?;
        }
        ...
        let key_size = pivot.remaining();
        let ok = self
            .insert_content_to_interior(&mut a, 0, pivot, b.id(), key_size)
            .await?;
        assert!(ok);
    ...
    async fn new_page(&self) -> anyhow::Result<PageWrite<'a, R>> {
        let Some(_freelist_pgid) = self.pager.read_state().await.freelist else {
            let page = self.pager.alloc(self.ctx, self.txid).await?;
            return Ok(page);
        };
    ...
    pub(crate) async fn get(&self, key: &[u8]) -> anyhow::Result<Option<GetResult<R>>> {
        let mut current = self.pager.read(&self.ctx, self.txid, self.root).await?;
        let page = loop {
            if !current.is_interior() {
                break current;
            }
            ...
            let (i, _) = self.search_key_in_interior(&node, key).await?;
            let next_pgid = if i == node.count() {
                node.last()
            } else {
                node.get(i).ptr()
        ...
        let (i, found) = self.search_key_in_leaf(&leaf, key).await?;
        if !found {
            return Ok(None);
        }
        Ok(Some(GetResult {
    ...
    async fn search_key_in_interior(
        &self,
        ...
        'a: 'b,
    {
        let (start, skip_first) = match range.start_bound() {
            Bound::Included(key) => (self.find_position(key).await?, false),
            Bound::Excluded(key) => (self.find_position(key).await?, true),
            Bound::Unbounded => (self.find_position(&[]).await?, false),
        };
        ...
        let end = match range.end_bound() {
            Bound::Included(key) => {
                let Some((page, i)) = self.find_position(key).await? else {
                    return Ok(Cursor::Empty);
                };
    ...
    async fn find_position(
        ...
        let (i, _) = self.search_key_in_leaf(&leaf, key).await?;
        Ok(Some((leaf, i)))
    }
}
...
impl<R: Runtime> GetResult<'_, R> {
    pub(crate) async fn get(self) -> anyhow::Result<KVItem> {
...
impl<'a, R: Runtime> Cursor<'a, R> {
    pub(crate) async fn next(&mut self) -> anyhow::Result<Option<KVItem>> {
...
impl Content for KeyValContent<'_> {
    ...
    async fn put(&mut self, mut target: &mut [u8]) -> anyhow::Result<()> {
